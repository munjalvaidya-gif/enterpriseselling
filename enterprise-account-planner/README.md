...
One of the [Enterprise Selling](../README.md) skills. A [Claude Skill](https://www.anthropic.com/news/skills) that turns three inputs — a target account, its vertical, and what you sell — into a full Force Management-style account plan with MEDDPICC baked in.
...
**claude.ai:** upload this `enterprise-account-planner/` folder (or the packaged `.skill` file, if one is included in [Releases](../../../releases)) via Settings → Capabilities → Skills.
...
Already have a plan for a deal? The companion [`objection-sparring-partner`](../objection-sparring-partner/) skill will read it in as context instead of asking you to re-describe the account.
...
Two full, real generated plans (live web research, no fabricated facts) are in [`examples/enterprise-account-planner/`](../examples/enterprise-account-planner/):

- [`nike-workday-account-plan.md`](../examples/enterprise-account-planner/nike-workday-account-plan.md) — a Workday seller targeting Nike (net-new land motion)
- [`marriott-salesforce-account-plan.md`](../examples/enterprise-account-planner/marriott-salesforce-account-plan.md) — a Salesforce seller targeting Marriott International (existing-customer expansion motion)

## Testing

This skill was validated with a formal benchmark, not just eyeballed: the same task was run with and without the skill across 4 real Fortune 500 accounts (different verticals and platform categories, including one platform — an AI coding agent — deliberately not covered by the persona playbook, to test the fallback reasoning), graded against objective checks (real cited executives, all 8 MEDDPICC elements present, persona correctly mapped to the seller's platform, no fabricated facts). See [`evals/enterprise-account-planner/`](../evals/enterprise-account-planner/) for the eval scenarios.
